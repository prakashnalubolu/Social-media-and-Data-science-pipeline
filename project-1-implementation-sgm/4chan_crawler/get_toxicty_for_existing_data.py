        ...
        while(True):
            ...
            if update_count > 0 :
                ...
                for id, thread_number, comment in rows:
                    if not comment: no_comment+=1
                    # print(id, thread_number, comment)
                ...
                # update_query = """
                #     UPDATE posts
                #     SET toxic_flag = CAST(data.toxic_flag AS BOOLEAN),
                ...
                logger.info(f"No comment count: {no_comment}")
                try: 
                    update_query = """
                    UPDATE posts
                    ...
                    logger.info(f"Error in update query - {e}")
